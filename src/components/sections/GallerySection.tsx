...
// Desktop: tre snören med 6 + 6 + 5 foton. Mobil: sex kortare snören med 3
// foton per rad, så att varje foto alltid hänger på ett synligt snöre i stället
// för att radbrytas under det.
function chunkImages(size: number): { row: typeof images; offset: number }[] {
  const result: { row: typeof images; offset: number }[] = [];
  for (let i = 0; i < images.length; i += size) {
    result.push({ row: images.slice(i, i + size), offset: i });
  }
  return result;
}

const desktopRows = chunkImages(6);
const mobileRows = chunkImages(3);
          ...
          return (
            <motion.li
              ...
              className={`relative list-none ${
                variant === "mobile"
                  ? "w-[6.75rem] sm:w-[8.25rem]"
                  : "md:w-[calc((100%-5rem)/6)] lg:w-[calc((100%-6.25rem)/6)]"
              }`}
              style={{
  ...
  return (
    <section id="galleri" className="smilo-scroll-anchor bg-smilo-cream pb-16 md:pb-24 lg:pb-32">
      <div className="smilo-container">
        ...
        {/* Mobil: sex korta snören med tre foton per rad */}
        <div className="mx-auto flex max-w-5xl flex-col gap-9 sm:gap-12 md:hidden">
          {mobileRows.map(({ row, offset }, rowIndex) => (
            <PhotoRow
              key={offset}
              row={row}
              offset={offset}
        ...
        {/* Desktop: tre långa snören med sex foton per rad */}
        <div className="mx-auto hidden max-w-5xl flex-col gap-14 md:flex lg:gap-16">
          {desktopRows.map(({ row, offset }, rowIndex) => (
            <PhotoRow
              key={offset}
              row={row}
              offset={offset}
